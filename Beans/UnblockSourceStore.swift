...
/// headers：可选的附加请求头；script：落雪 LX 脚本音源（kind == "lxscript"）的完整 JS 源码
/// enabled：是否启用（用户可独立开关，默认开启）
struct ThirdPartySource: Identifiable, Codable, Hashable {
    ...
    enum CodingKeys: String, CodingKey { case id, name, kind, template, urlPath, headers, script, enabled }

    init(id: String = UUID().uuidString, name: String, kind: String = "keyword", template: String, urlPath: String = "url", headers: [String: String] = [:], script: String = "", enabled: Bool = true) {
        self.id = id
        self.name = name
        self.kind = kind
...
/// 第三方解锁源管理：仅用户导入的自定义源（UserDefaults 持久化）
final class UnblockSourceStore: ObservableObject {
    ...
    func setEnabled(_ source: ThirdPartySource, enabled: Bool) {
        guard let idx = customSources.firstIndex(where: { $0.id == source.id }) else { return }
        customSources[idx].enabled = enabled
    }
