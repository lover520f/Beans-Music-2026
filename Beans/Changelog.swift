    ...
    static let logs: [VersionLog] = [
        VersionLog(
            ...
            fixes: [
                ...
                "移除「大封面模式」功能（含播放器设置入口与相关联动），封面恢复为常规居中布局",
            ],
        ),
        VersionLog(
            id: "1.5.8",
        ...
        VersionLog(
            ...
            features: [
                "新增「大封面模式」（播放器设置 → 封面）：开启后播放页封面放大撑满，并自动隐藏底部预览歌词",
                "移除汽水音乐全部功能（登录 / 歌单同步 / 播放）：清除相关入口与界面",
            ],
            fixes: [
                "修复 QQ 空歌单误报加载失败：真实空歌单显示为「暂无歌曲」，不再误报登录或版权错误",
                "修复 QQ「我的喜欢」歌单无法加载：兼容 map 字段数字 / 字符串 / 嵌套多种类型，解析失败记录详细日志",
                "修复 QQ 歌单列表偶发为空：创建歌单请求登录态失败时自动用游客方式重试",
                "清除汽水音乐相关代码，此前无法正常登录与同步的汽水功能一并移除",
            ],
        ),
        VersionLog(
            id: "1.5.5",
            version: "1.5.5",
            title: "音频混合 / QQ歌单修复 / 汽水歌单修复",
            features: [
                "新增不打断其他音频开关（默认开启）：与其他 App 音频混合播放",
                "新增灵动岛实时活动开关（默认开启，iOS 16.1+ 生效）：播放时显示正在播放歌曲",
                "汽水音乐登录改为抖音护照扫码登录：应用内直接显示二维码，用抖音 APP 扫码授权后自动同步歌单",
            ...
            fixes: [
                ...
                "修复 QQ 歌单歌曲加载失败被误报为未登录：登录态 hostUin 参数优化 + 报错文案更清晰",
                "修复 QQ 登录后歌单不显示：新增 musicu GetUserPlaylist 兜底接口",
                "修复汽水音乐扫码登录偶发无反应：补齐抖音护照全量 Cookie 与状态码解析",
