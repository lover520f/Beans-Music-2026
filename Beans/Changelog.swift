...
enum ChangelogStore {
    ...
    static let logs: [VersionLog] = [
        ...
        VersionLog(
            ...
            features: [
                "播放器底部控制区新增 iOS 26 原生液态玻璃底栏：进度条 / 控制按钮 / 指示线统一收纳进玻璃底栏，歌词滑入底栏下方后由玻璃层模糊遮盖，不再清晰透出",
                "底栏跟随全局 液态玻璃 / 磨砂玻璃 设置，深浅模式自适应，歌词显示多少调节保持不变",
            ],
            fixes: []
        ),
        VersionLog(
            id: "1.9.16",
            version: "1.9.16",
            title: "恢复歌词显示多少调节 · 歌词位置恢复可调",
            features: [
                "恢复「歌词显示多少」上下留白调节：布局调整弹窗与播放器设置中的对应滑杆重新可用，歌词位置 / 显示行数恢复可自由调整",
                "歌词视口留白重新跟随用户设定值，歌词页上下显示恢复原有效果",
            ],
            fixes: []
        ),
        VersionLog(
            id: "1.9.15",
            version: "1.9.15",
            title: "播放器底部恢复原始布局 · 移除歌词显示多少",
            features: [
                "播放器底部组件（进度条 / 控制按钮 / 指示线）恢复最初始的默认布局，不再使用自定义偏移与缩放",
                "移除「歌词显示多少」上下留白调节：布局调整弹窗与播放器设置中的对应滑杆一并删除，歌词视图恢复原始展示",
            ],
            fixes: []
        ),
        VersionLog(
