...
### 新增
- 新增「音乐服务器」接入：在电脑上运行 BeansMusicServer（基于 musicdl 的多平台音源接口），搜索歌曲时自动把服务器解析出的可播放结果置顶，可直接播放（含部分 VIP / 版权歌）
- 新增「音乐服务器」设置项：填写服务器地址即可一键测试连接（我的 → 设置 → 音乐服务器）
- 新增服务器内置网页版搜索即播测试页（GET /），便于电脑上直接搜索验证
